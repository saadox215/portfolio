export const Menu = ({ onSectionChange, menuOpened, setMenuOpened }) => <>
  ...
  <nav className={`side-menu ${menuOpened ? "opened" : ""}`} aria-label="Main navigation">
    <div className="menu-inner"><p className="eyebrow">SAAD AFIFI / NAVIGATION</p><MenuButton label="About" onClick={() => onSectionChange(1)} number="01" /><MenuButton label="Toolkit" onClick={() => onSectionChange(2)} number="02" /><MenuButton label="Work" onClick={() => onSectionChange(3)} number="03" /><MenuButton label="Contact" onClick={() => onSectionChange(4)} number="04" /></div>
  </nav>
</>;
